...
for i = 1:m
  z_2(i,:) = Theta1 * X(i,:)';
  a_2(i,:) = sigmoid(z_2(i,:));
  z_3(i,:) = Theta2 * ([1;a_2(i,:)']);
  h_theta(i,:) = sigmoid(z_3(i,:))';
  y_vec(i,y(i))=1;
  J = J + (((-1.*y_vec(i,:)')' * log(h_theta(i,:)'))- ((1-y_vec(i,:)')'*(log(1-h_theta(i,:)'))));
end
J = (J + (lambda/2)*(sum(sum(Theta1(:,2:input_layer_size + 1).^2))+(sum(sum(Theta2(:,2:hidden_layer_size+1).^2)))))/m;
...
err_2 = zeros(m,hidden_layer_size);
delta_accum_1 = zeros(size(Theta1));
delta_accum_2 = zeros(size(Theta2));

for i = 1:m
  err_3(i,:) = h_theta(i,:)-y_vec(i,:);
  err_2(i,:) = (err_3(i,:)*Theta2(:,2:hidden_layer_size+1)).*sigmoidGradient(z_2(i,:));
  delta_accum_1 = delta_accum_1 + err_2(i,:)' * X(i,:);
  delta_accum_2 = delta_accum_2 + err_3(i,:)' * [1 a_2(i,:)];
end
